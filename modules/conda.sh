...
detect_conda_installation() {
    ...
    if [[ -d "$CONDA_PATH" && -x "$CONDA_PATH/bin/conda" ]]; then
        ...
        log_success "Found system conda installation: $CONDA_PATH"
        return 0
    fi
    
    # If not found, install conda
    log_info "No conda installation found at $CONDA_PATH"
    ...
    if [[ -d "$CONDA_PATH" && -x "$CONDA_PATH/bin/conda" ]]; then
        export DETECTED_CONDA_PATH="$CONDA_PATH"
        export CONDA_TYPE="system"
        log_success "Conda installed and detected at $CONDA_PATH"
        return 0
    else
...
install_conda() {
    ...
    if ! wget -q -O /tmp/micromamba.tar.bz2 --user-agent="Mozilla/5.0" "$micromamba_url"; then
...
update_conda() {
    ...
    if "$conda_cmd" update -n base -c defaults conda -y; then
        ...
        if "$conda_cmd" install -n base -c defaults conda --force-reinstall -y; then
...
set_conda_environment() {
    ...
    if [[ "${CONDA_DISABLE_LIBMAMBA:-true}" == "true" ]]; then
    ...
    if [[ "${CONDA_NO_PLUGINS:-true}" == "true" ]]; then
        export CONDA_NO_PLUGINS=true
        log_debug "Conda plugins disabled"
    fi
    
    if [[ "${CONDA_REPORT_ERRORS:-false}" == "false" ]]; then
        export CONDA_REPORT_ERRORS=false
    fi
    
    if [[ "${ANACONDA_ANON_USAGE:-false}" == "false" ]]; then
...
show_conda_solver_config() {
    ...
    if [[ "$CONDA_DISABLE_LIBMAMBA" == "true" ]]; then
...
configure_conda_channels() {
    ...
    for ((i=${#channels_array[@]}-1; i>=0; i--)); do
    ...
    CONDA_NO_PLUGINS=true conda config --show channels | sed 's/^/  /'
...
accept_conda_terms() {
    ...
    if echo "$CONDA_CHANNELS" | grep -q "defaults\|anaconda\|repo.anaconda.com"; then
        ...
        for channel in "${channels_to_accept[@]}"; do
...
install_conda_base_packages() {
    ...
    # The script now checks GLOBAL_INSTALL_PROFILE instead of CONDA_PROFILE.
    log_info "Using GLOBAL_INSTALL_PROFILE: '${GLOBAL_INSTALL_PROFILE}'"
    ...
    case "${GLOBAL_INSTALL_PROFILE}" in
        "standard")
            packages_to_install+=("${CONDA_DATA_PACKAGES[@]}")
            ;;
        "full")
            packages_to_install+=("${CONDA_DATA_PACKAGES[@]}")
            packages_to_install+=("${CONDA_DEV_PACKAGES[@]}")
            packages_to_install+=("${CONDA_OPTIONAL_PACKAGES[@]}")
            packages_to_install+=(${CONDA_ML_PACKAGES})
            ;;
    esac
    # --- END OF CHANGE ---
    ...
    micromamba create -y -p "${CONDA_PATH}/envs/${env_name}" \
        --channel pytorch \
        --channel nvidia \
        --channel conda-forge \
        "${unique_packages[@]}" || {
    ...
    log_info "To activate this environment, run: micromamba activate ${env_name}"
    return 0
}
...
install_conda_dev_packages() {
    ...
    for package in "${CONDA_DEV_PACKAGES[@]}"; do
        if ! "$conda_cmd" list -n base | grep -q "^$package "; then
...
configure_conda_for_users() {
    ...
    setup_conda_permissions || {
        log_error "Failed to setup conda permissions"
        return 1
    }
...
configure_conda_user_init() {
    ...
    if [[ -f "$bashrc_file" ]] && grep -q "micromamba shell init" "$bashrc_file" && grep -q "MAMBA_ROOT_PREFIX='$CONDA_PATH'" "$bashrc_file"; then
...
setup_conda_permissions() {
    log_debug "Setting up conda permissions..."
    
    if [[ "${DRY_RUN:-false}" == "true" ]]; then
        log_info "[DRY RUN] Would setup conda permissions"
        return 0
    fi
    
    # Make conda accessible to development group
    if [[ "$CONDA_TYPE" == "system" ]]; then
        log_debug "Configuring system conda permissions"
        
        # Set group ownership to development group
        log_debug "Setting conda group ownership to: $DEV_GROUP"
        chgrp -R "$DEV_GROUP" "$DETECTED_CONDA_PATH" 2>/dev/null || {
            log_warn "Failed to change conda group ownership"
        }
        
        # Set group read/execute permissions on directories
        log_debug "Setting directory permissions..."
        find "$DETECTED_CONDA_PATH" -type d -exec chmod g+rx {} \; 2>/dev/null || {
            log_warn "Failed to set conda directory permissions"
        }
        
        # Set group read/execute permissions on executables
        log_debug "Setting executable permissions..."
        find "$DETECTED_CONDA_PATH" -type f -executable -exec chmod g+rx {} \; 2>/dev/null || {
            log_warn "Failed to set conda executable permissions"
        }
        
        # Make conda writable for package installations
        local writable_dirs=("pkgs" "envs" "conda-meta" "var")
        for dir in "${writable_dirs[@]}"; do
            local full_path="$DETECTED_CONDA_PATH/$dir"
            if [[ -d "$full_path" ]]; then
                log_debug "Setting write permissions for: $full_path"
                chmod -R g+w "$full_path" 2>/dev/null || {
                    log_debug "Could not set write permissions for: $full_path"
                }
            fi
        done
        
        # Ensure conda binary is executable by group
        chmod g+rx "$DETECTED_CONDA_PATH/bin/conda" 2>/dev/null || {
            log_warn "Failed to set conda binary permissions"
        }
        
        # Verify the user is in the development group
        if ! groups "$DEV_USERNAME" | grep -q "\b$DEV_GROUP\b"; then
            log_warn "User $DEV_USERNAME is not in group $DEV_GROUP"
            log_info "Adding user to development group..."
            usermod -a -G "$DEV_GROUP" "$DEV_USERNAME" || {
                log_error "Failed to add user to development group"
                return 1
            }
        fi
        
    else
        log_debug "User conda installation, permissions should be correct"
    fi
    
    log_debug "Conda permissions setup completed"
    return 0
}
# Update the create_user_conda_config function (around line 354):
...
verify_user_conda_access() {
    ...
    # in the environment we created for them. This proves the setup is correct.
    ...
    if sudo -u "$DEV_USERNAME" bash -c "${test_cmd}" >/dev/null 2>&1; then
...
verify_conda_packages() {
    ...
    for package in "${CONDA_BASE_PACKAGES[@]}"; do
        if ! echo "${installed_packages}" | grep -q -w "${package}"; then
...
show_conda_info() {
    ...
    if [[ "${DRY_RUN:-false}" != "true" ]] && [[ -n "${DETECTED_CONDA_PATH:-}" ]]; then
        echo "  Version: $(micromamba --version 2>/dev/null || echo "Unknown")"
        
        echo ""
        echo "Created Environment:"
        micromamba env list | sed 's/^/  /' || echo "  Unable to list environments"
        
        echo ""
        echo "Packages in '${env_name}' Environment (Top 10):"
        micromamba list -p "${env_path}" | head -n 12 | sed 's/^/  /' || echo "  Unable to list packages"
