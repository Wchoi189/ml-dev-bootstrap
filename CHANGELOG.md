...
### 추가됨
- **로컬 구성 재정의**: `config/local.conf`를 통해 git 추적 없이 개인 설정을 재정의할 수 있는 기능 추가 (`config/local.conf.example` 참조).
- **중앙 집중식 컨텍스트**: `lib/context.sh`를 통해 프로젝트 루트 경로 및 설정 로딩 로직을 중앙화하여 경로 의존성 문제 해결.
- **플랫폼 감지**: `lib/platform.sh`를 통해 OS 및 배포판(Ubuntu, Debian 등)을 견고하게 감지하는 로직 추가.
- **인증 유틸리티**: `utils/auth.sh`를 통해 GitHub 토큰 감지 및 인증 로직을 중앙화하고 여러 모듈(`ghcli`, `git`)에서 재사용.

### 변경됨
- **모듈 표준화**: 모든 모듈이 `lib/bootstrap_module.sh`를 통해 공통 컨텍스트와 로깅을 로드하도록 표준화.
- **엄격한 모드**: 모든 스크립트에 `set -euo pipefail`을 적용하여 오류 발생 시 즉시 중단되도록 안정성 강화.
- **코드 구조**: `setup.sh` 및 각 모듈에서 중복되는 경로 계산 및 설정 로딩 코드를 제거.
